...
function runMaude(opts) {
  const spawn = opts.spawn ?? ((cmd, args, o) => (0, import_node_child_process.spawn)(cmd, args, o));
  return new Promise((resolve) => {
    const child = spawn(
      opts.maudePath,
      ["-no-banner", "-batch", "-no-wrap", opts.filePath],
      { cwd: opts.cwd }
    );
    let stdout = "";
    let stderr = "";
    ...
    const finish = () => {
      ...
      settled = true;
      clearTimeout(timer);
      resolve({ stdout, stderr, timedOut, error: errorMessage });
    };
    child.stdout?.on("data", (c) => stdout += c.toString());
    child.stderr?.on("data", (c) => stderr += c.toString());
    child.on("error", (err) => {
      errorMessage = err instanceof Error ? err.message : err === void 0 ? "spawn error" : String(err);
      finish();
    });
...
// server/src/diagnostics/parseOutput.ts
var import_node = __toESM(require_node3());
var LINE_RE = /^(Warning|Advisory|Error):\s+"[^"]*"(?:,\s*line\s+(\d+))?[^:]*:\s*(.*)$/;
function parseMaudeMessages(output) {
  const messages = [];
  for (const raw of output.split(/\r?\n/)) {
    ...
    messages.push({
      kind: m[1],
      line: m[2] ? parseInt(m[2], 10) : void 0,
      message: m[3].trim()
    });
  }
  return messages;
}
var ERROR_HINT = /no parse|bad token|error|undefined|incompatible|cannot|does not|missing/i;
function severityFor(msg) {
  if (ERROR_HINT.test(msg.message)) return import_node.DiagnosticSeverity.Error;
  if (msg.kind === "Advisory") return import_node.DiagnosticSeverity.Information;
  return import_node.DiagnosticSeverity.Warning;
}
function messagesToDiagnostics(messages, lineText) {
  return messages.map((m) => {
    const line0 = Math.max(0, (m.line ?? 1) - 1);
    let range;
    if (m.line !== void 0) {
      const text = lineText(line0) ?? "";
      const startCh = text.length - text.trimStart().length;
      const endCh = text.length;
      range = {
        start: { line: line0, character: startCh },
        end: { line: line0, character: endCh }
      };
    } else {
      range = {
      ...
      return [
        {
          ...
          source: "maude",
          message: `maude could not be started: ${result.error}. Check the "maude.path" setting.`
        }
      ];
    }
    if (result.timedOut) {
      return [
        {
          ...
          source: "maude",
          message: `maude diagnostics timed out after ${opts.timeoutMs} ms`
        }
      ];
    }
...
${result.stderr}`;
    ...
    const messages = parseMaudeMessages(combined);
    return messagesToDiagnostics(messages, (l) => lines[l]);
  } finally {
    (0, import_node_fs.rmSync)(dir, { recursive: true, force: true });
  }
}

// server/src/symbols/documentSymbols.ts
var import_node3 = __toESM(require_node3());
var MODULE_START = /^\s*(fmod|mod|smod|omod|fth|th|sth|oth|view)\s+(\S+)/;
var MODULE_END = /^\s*(endfm|endm|endsm|endom|endo|endfth|endth|endsth|endoth|endv)\b/;
var DECL = /^\s*(sorts?|subsorts?|ops?|eq|ceq|rl|crl|mb|cmb|vars?|msgs?|class|subclass|strats?|sd|csd)\b\s*(.*)$/;
function lineRange(line, text) {
  return {
    start: { line, character: 0 },
    end: { line, character: text.length }
  };
}
function declName(rest) {
  let r = rest.replace(/\s*\.\s*$/, "").trim();
  const cut = r.search(/[:=]/);
  if (cut > 0) r = r.slice(0, cut).trim();
  return r;
}
function declKind(kw) {
  if (kw.startsWith("sort") || kw.startsWith("subsort") || kw === "class" || kw === "subclass")
    return import_node3.SymbolKind.Class;
  if (kw.startsWith("op")) return import_node3.SymbolKind.Function;
  if (kw.startsWith("var")) return import_node3.SymbolKind.Variable;
  if (kw.startsWith("msg")) return import_node3.SymbolKind.Event;
  return import_node3.SymbolKind.Method;
}
function computeDocumentSymbols(text) {
  const lines = text.split(/\r?\n/);
  const roots = [];
  let current = null;
  lines.forEach((line, i) => {
    if (!current) {
      const mStart = MODULE_START.exec(line);
      if (mStart) {
        const sym = {
          name: mStart[2],
          detail: mStart[1],
          kind: mStart[1] === "view" ? import_node3.SymbolKind.Interface : import_node3.SymbolKind.Module,
          range: lineRange(i, line),
          selectionRange: lineRange(i, line),
          children: []
        };
        roots.push(sym);
        current = { sym, startLine: i };
      }
      return;
    }
    if (MODULE_END.test(line)) {
      current.sym.range = {
        start: { line: current.startLine, character: 0 },
        end: { line: i, character: line.length }
      };
      current = null;
      return;
    }
    const mDecl = DECL.exec(line);
    if (mDecl) {
      const name = declName(mDecl[2]);
      if (name) {
        current.sym.children.push({
          name,
          detail: mDecl[1],
          kind: declKind(mDecl[1]),
          range: lineRange(i, line),
          selectionRange: lineRange(i, line)
        });
      }
...
// server/src/folding/foldingRanges.ts
var import_node4 = __toESM(require_node3());
var MODULE_START2 = /^\s*(fmod|mod|smod|omod|fth|th|sth|oth|view)\s+\S+/;
var MODULE_END2 = /^\s*(endfm|endm|endsm|endom|endo|endfth|endth|endsth|endoth|endv)\b/;
var BLOCK_OPEN = /(\*\*\*|---)\(/;
...
function computeFoldingRanges(text) {
  ...
  lines.forEach((line, i) => {
    if (blockStart !== null) {
      ...
      if (depth <= 0) {
        if (i > blockStart) {
          ranges.push({ startLine: blockStart, endLine: i, kind: import_node4.FoldingRangeKind.Comment });
        }
        blockStart = null;
        depth = 0;
      }
    ...
    const openIdx = line.search(BLOCK_OPEN);
    if (openIdx >= 0) {
      const fromOpener = line.slice(openIdx + line.slice(openIdx).indexOf("("));
      depth = countParens(fromOpener);
      if (depth > 0) {
        blockStart = i;
      } else {
        depth = 0;
    ...
    if (moduleStart === null && MODULE_START2.test(line)) {
      moduleStart = i;
      return;
    }
    if (moduleStart !== null && MODULE_END2.test(line)) {
      if (i > moduleStart) ranges.push({ startLine: moduleStart, endLine: i });
      moduleStart = null;
    }
  });
  return ranges;
...
connection.onInitialized(async () => {
  const cfg = await connection.workspace.getConfiguration({ section: "maude" }).catch(() => void 0);
  if (cfg) {
    settings = {
      path: cfg.path ?? "maude",
      diagnostics: {
  ...
  timers.set(
    ...
    setTimeout(async () => {
      ...
      try {
        ...
        connection.console.error(`maude diagnostics failed: ${String(err)}`);
      }
    }, 400)
  );
}
...
documents.onDidClose((e) => {
  const pending = timers.get(e.document.uri);
  if (pending) {
